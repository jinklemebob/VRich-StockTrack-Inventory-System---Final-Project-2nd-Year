...
namespace FinalProject2ndYear
{
    public partial class editGoodsReceiptForm : Form
    {
        ...
        private void editGoodsReceiptForm_Load(object sender, EventArgs e)
        {
            LoadSuppliers();
            InitializeItemsGrid();
            LoadReceiptDetails();
        ...
        private void InitializeItemsGrid()
        {
            ...
            expiryCol.HeaderText = "Expiry Date (MM/DD/YYYY)";
            expiryCol.Width = 120;
        ...
        {
            string query = @"
                SELECT gr.SupplierID, gr.ReferenceNo, gr.ReceiptDate
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            SupplierComboBox.SelectedValue = Convert.ToInt32(reader["SupplierID"]);
                            ReferenceNoTextBox.Text = reader["ReferenceNo"].ToString();
                            ReceiptDatePicker.Value = Convert.ToDateTime(reader["ReceiptDate"]);
                        }
                    }
                }
            }
        ...
        {
            string query = @"
                SELECT gri.ProductID, gri.Qty, sb.ExpiryDate
                ...
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            ItemsDataGrid.Rows[rowIndex].Cells["ExpiryDate"].Value = Convert.ToDateTime(reader["ExpiryDate"]).ToString("MM/dd/yyyy");
                        }
                    }
                }
            }
        ...
        private void SubmitButton_Click(object sender, EventArgs e)
        {
            ...
            // Check if receipt has been used in Sales Orders
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                conn.Open();
                using (SqlCommand check = new SqlCommand(@"
                    SELECT COUNT(*) FROM StockOuts so
                    JOIN StockBatches sb ON so.BatchID = sb.BatchID
                    JOIN GoodsReceiptItems gri ON sb.ReceiptItemsID = gri.ReceiptItemsID
                    WHERE gri.ReceiptID = @ReceiptID", conn))
                {
                    check.Parameters.AddWithValue("@ReceiptID", receiptID);
                    int count = (int)check.ExecuteScalar();
                    if (count > 0)
                    {
                        MessageBox.Show("This receipt cannot be edited because its stock has already been used in a Sales Order.",
            ...
            {
                conn.Open();
                using (SqlCommand cmd = new SqlCommand("sp_EditGoodsReceipt", conn))
                {
                    ...
                    {
                        cmd.ExecuteNonQuery();
                        MessageBox.Show("Goods Receipt updated!", "Successful",
